...
async def start(bot, message):
    ...
    if len(message.command) > 1 and message.command[1] == 'subscribe':
        await message.reply('⚠️ You are not Subscribed @SBS_Studio. \n\nPlease Join @SBS_Studio channel')
    else:
        buttons = [[
            InlineKeyboardButton('Search Here 🔎', switch_inline_query_current_chat=''),
            InlineKeyboardButton('Channel 🗣', url='https://t.me/SBS_Studio'),
        ]]
...
def help_user(bot, update):
    ...
    bot.send_message(
        chat_id=update.chat.id,
        text='Join Now . . .',
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="JOIN NOW", url=f"telegram.me/{MY_CHANNEL}")]]),
        parse_mode="html",
        disable_web_page_preview=True,
        reply_to_message_id=update.message_id
    )
